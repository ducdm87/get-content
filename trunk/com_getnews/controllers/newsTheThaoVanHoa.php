 ...
 *
 */
function mosControllerTheThaoVanHoaGetNews()
{	
	$number	=	mosGetNumberRow('#__article2010_new_thethaovanhoa');
	echo $number.' rows';
	if ($number > 30000) {
		echo 'database is very big.(article2010_new_thethaovanhoa)';
		echo 'Please backup table and try agein.';
		die();
	}
	global $arrErr;	
	require_once(dirname(__FILE__).DS.'..'.DS.'configs'.DS.'thethaovanhoa.php');
	
	if (isset($_REQUEST['live'])) {
		$lastGet_vovId	=	0;
	}else {
		require_once(dirname(__FILE__).DS.'..'.DS.'cache'.DS.'helpers.php');
		$cache_file	=	dirname(__FILE__).DS.'..'.DS.'cache'.DS.'data'.DS.'thethaovanhoa.php';
	...
	$obj_cat	=	mosModelTheThaoVanHoaGetNews($get_existing);	
 ...
 */
function mosControllerTheThaoVanHoaGetTTVH()
{
	...
	$data = mosModelTheThaoVanHoaGetTTVH($cat_parent,$cat_id,$id_content,$alias_content, $section_id, $catid, $path_image, $link_image, $SiteID);
