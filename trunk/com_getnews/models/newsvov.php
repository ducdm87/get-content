...
function mosModelNewsvovGetNews(& $lastGet_vovId, $max_number = 1000, $get_existing = true)
{		
	...
	while ($bool and $number_run <= 10 and $id_content > $_REQUEST['end_id']) {
		echo '$id_result: '.$id_result.' | '.'$id_content: '.$id_content.' | $number_run: '.$number_run;
		$arr_ID			=	array();
		...
		if ($get_existing == false) {
			...
			$arr_geted	=	mosGetDBOBJ('#__article2010_totalcontent',array('id_origional'),'id_origional  in('.$_id.') AND SiteID='.$db->quote('vn10'));
...
//				die();
			...
			echo ' $arr_ID: '. count($arr_ID);			
			echo ' $arr_result: '. count($arr_result);
			echo ' &nbsp; '.($bool).' <br />';
	...
	$fp = fopen( dirname(__FILE__).DS.'..'.DS.'log'.DS.'vov_logfile.txt', 'a');
	fputs($fp, "__________________________________________________\r\n");	
	fputs($fp, "\t\t time: $now \r\n");
	fputs($fp, "\t\t id_content: ".$_REQUEST['id_content']."\r\n");
	fputs($fp, "\t\t id_result: $id_result\r\n");
	fclose($fp);
	$i = 0;
	$number_getContent	=	0;
...
function mosModelNewsvovGetNewId()
{
	...
	$list_news	=	$html->find('div[id="spotlight"]',0);
	if ($spotlights	=	$list_news->find('div[class="spotlights"]',0)) {
		$items	=	$spotlights->find('h1[class="title"]');
		for ($i=0;$i<count($items); $i++)
		{
			$link	=	$items[$i]->first_child()->href;
			if (!preg_match($reg,$link,$matches)) {				
				continue;				
			}
			$arr_ID[]	=	intval($matches[1]);
		}
	}
	if ($latest = $list_news->find('div[class="latest"]',0)) {
		$items	=	$latest->find('ul[class="story-listing"]',0)->children();
		for ($i=0;$i<count($items); $i++)
		{
			$link	=	$items[$i]->first_child()->href;
			if (!preg_match($reg,$link,$matches)) {				
