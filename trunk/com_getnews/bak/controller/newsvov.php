...
{	
	...
	if (isset($_REQUEST['id_content'])) {
		...
		$lastGet_vovId	=	$cache_helper->getlastGet_Id();	
		$startId		=	$cache_helper->getStartID();		
	}else {
		require_once(dirname(__FILE__).DS.'..'.DS.'cache'.DS.'helpers.php');
		$cache_file	=	dirname(__FILE__).DS.'..'.DS.'cache'.DS.'data'.DS.'vov_cache.php';
		$cache_helper	=	new cacheHelper($cache_file);
	...
	$obj_result	=	mosModelNewsvovGetNews($startId, $lastGet_vovId,$numbercontent,false);	
	...
	if (!isset($_REQUEST['id_content']))
	{
		if ($id_result == 0 || $id_result < $lastGet_vovId) {
			$lastGet_vovId 	= 	$startId;
			$startId		=	0;
		}
		
		$cache_helper->setStartID($startId);
