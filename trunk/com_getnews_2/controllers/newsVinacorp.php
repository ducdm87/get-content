 ...
 *
 */
function mosControllerNewsVinacorpGetNews()
{
	$number	=	mosGetNumberRow('#__article2010_new_vinacorp');
	echo $number.' rows';
	if ( $number > 30000) {
		echo '<br />database is very big.(article2010_new_vinacorp)';
		echo 'Please backup table and try again.';
		die();
	}
	global $arrErr;	
	require_once(dirname(__FILE__).DS.'..'.DS.'configs'.DS.'vinacorp.php');
	...
	$obj_cat	=	mosModelNewsVinacorpGetNews($get_existing);	
 ...
 */
function mosControllerNewsnewsVinacorpGetVNC($path_image = 'images/vnc355/')
{
	...
	mosModelNewsVinacorpGetCNC($id_content, $content_link, $content_title, $content_alias, $page, $secid, $catid, $catid_vnc, $path_image, $link_image, $SiteID);
