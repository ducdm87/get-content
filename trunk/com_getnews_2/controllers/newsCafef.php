 ...
 *
 */
function mosControllerNewsCafefGetNews()
{
	$number	=	mosGetNumberRow('#__article2010_new_cafef');
	echo $number.' rows';
	if ( $number > 30000) {
		echo '<br />database is very big.(article2010_new_cafef)';
		echo 'Please backup table and try again.';
		die();
	}
	global $arrErr;	
	require_once(dirname(__FILE__).DS.'..'.DS.'configs'.DS.'cafef.php');
	...
	$obj_cat	=	mosModelNewsCafefGetNews($get_existing);	
 ...
 */
function mosControllerNewsnewsCafefGetC2F($path_image = 'images/c2f345/')
{
	...
	mosModelNewsCafefGetC2F($id_content, $content_link, $content_title, $content_alias, $page, $secid, $catid, $catid_origional, $path_image, $link_image, $SiteID);
