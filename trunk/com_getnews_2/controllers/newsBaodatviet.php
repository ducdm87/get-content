...
function mosControllerNewsBDVGetNews()
{
	$number	=	mosGetNumberRow('#__article2010_new_baodatviet');
	echo $number.' rows';
	if ($number > 30000) {	
		echo '<br /> database is very big.(article2010_new_baodatviet)';
		echo 'Please backup table and try again.';
		die();
	}
	global $arrErr;	
	require_once(dirname(__FILE__).DS.'..'.DS.'configs'.DS.'baodatviet.php');
	...
	$obj_cat	=	mosModelBDVGetData($get_existing);	
...
function mosControllerNewsBaodatvietGetBDV()
{
	global $arrErr,$mosConfig_absolute_path,$mosConfig_live_site;
	require_once(dirname(__FILE__).DS.'..'.DS.'configs'.DS.'baodatviet.php');	
	
	$cat_alias	=	$_REQUEST['catalias_origional'];	
	...
	if (!$id_content) {
		echo $begin;
			echo 'ERROR_GET_CONTENT_PHAPLUATXAHOI|Not get content id';
		echo $end;
		die();
	}		
	...
	mosModelNewsBaodatvietGetBDV($id_content, $link_content, $alias_content, $title_content, $cat_alias, $secid, $catid, $path_image, $link_image, $SiteID);
	
	if (count($arrErr)) {
		echo $begin;
		echo 'ERROR_GET_CONTENT_PHAPLUATXAHOI|';
