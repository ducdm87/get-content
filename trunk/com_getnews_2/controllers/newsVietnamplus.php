 ...
 *
 */
function mosControllerNewsVietnamplusGetNews()
{
	$number	=	mosGetNumberRow('#__article2010_new_vietnamplus');
	echo $number.' rows';
	if ( $number > 30000) {
		echo '<br />database is very big.(article2010_new_vietnamplus)';
		echo 'Please backup table and try again.';
		die();
	}
	global $arrErr;	
	require_once(dirname(__FILE__).DS.'..'.DS.'configs'.DS.'vietnamplus.php');
	...
	$obj_cat	=	mosModelNewsVietnamplusGetNews($get_existing);	
 ...
 */
function mosControllerNewsnewsVietnamplusGetVNP($path_image = 'images/vnp330/')
{
	...
	if (!$id_content) {
		echo $begin;
			echo 'ERROR_GET_CONTENT_VIETNAMPLUS|Not get content id';
		echo $end;
		die();
	}		
	...
	mosModelNewsVietnamplusGetVNP($id_content, $content_link, $content_title, $content_alias, $page, $secid, $catid, $vnp_catid, $path_image, $link_image, $SiteID);
	
	if (count($arrErr)) {
		echo $begin;
		echo 'ERROR_GET_CONTENT_NGUOI_DUA_TIN|';
