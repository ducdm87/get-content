 ...
 *
 */
function mosControllerNewsEnthanhNienGetNews()
{	
	$number	=	mosGetNumberRow('#__article2010_new_thanhnien_en');
	echo $number.' rows';
	if ( $number > 30000) {
		echo 'database is very big.(article2010_new_thanhnien_en)';
		echo 'Please backup table and try agein.';
		die();
	}
	global $arrErr;	
	require_once(dirname(__FILE__).DS.'..'.DS.'configs'.DS.'en.thanhnien.php');
	
	if (isset($_REQUEST['live'])) {
		$lastGet_vovId	=	0;
	}else {
		require_once(dirname(__FILE__).DS.'..'.DS.'cache'.DS.'helpers.php');
		$cache_file	=	dirname(__FILE__).DS.'..'.DS.'cache'.DS.'data'.DS.'en_thanhnien_cache.php';
	...
	$obj_cat	=	mosModelNewsEnthanhNienGetNews($date_started, $get_existing);	
 ...
 */
function mosControllerNewsEnthanhNienGetVNN()
{
	...
	$year_alias	=	$_REQUEST['year_alias'];
	...
	mosModelNewsEnthanhNienGetTN($cat_title, $year_alias,$id_content, $section_id, $catid, $path_image, $link_image, $SiteID);
