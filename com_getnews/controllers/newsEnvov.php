 ...
 *
 */
function mosControllerNewsEnvovGetNews()
{	
	$number	=	mosGetNumberRow('#__article2010_new_vov_en');
	echo $number.' rows';
	if ($number > 30000) {
		echo 'database is very big.(article2010_new_vov_en)';
		echo 'Please backup table and try again.';
		die();
	}
	global $arrErr, $arrNotice;	
	require_once(dirname(__FILE__).DS.'..'.DS.'configs'.DS.'en.vov.php');
	require_once(dirname(__FILE__).DS.'..'.DS.'cache'.DS.'helpers.php');
	$cache_file	=	dirname(__FILE__).DS.'..'.DS.'cache'.DS.'data'.DS.'en_vov_cache.php';
...
//	die();
	...
	$obj_result	=	mosModelNewsEnvovGetNews( $lastGet_vovId,$numbercontent,false);
	$id_result	=	$obj_result->id_result;
	$end	=	date('Y-m-d H:i:s');
	echo '<hr />'.$start.' => '.$end.'<hr />';
	@set_time_limit($defalutExecution);
	$now = date('Y-m-d H:i:s');
	if (!isset($_REQUEST['id_content']) or $_REQUEST['id_content']	==	0) 
 ...
 */
function mosControllerNewsEnvovGetVOV()
{
	...
	mosModelNewsEnvovGetVOV($id_content, $section_id, $catid, $path_image, $link_image, $SiteID);
