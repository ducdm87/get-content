 ...
 *
 */
function mosControllerNewsNguoiduatinGetNews()
{
	$number	=	mosGetNumberRow('#__article2010_new_nguoiduatin');
	echo $number.' rows';
	if ($number > 30000) {	
		echo '<br /> database is very big.(article2010_new_nguoiduatin)';
		echo 'Please backup table and try again.';
		die();
	}
	global $arrErr;	
	require_once(dirname(__FILE__).DS.'..'.DS.'configs'.DS.'nguoiduatin.php');
	...
	$obj_cat	=	mosModelNewsNguoiduatinGetNews($get_existing);	
 ...
 */
function mosControllerNewsNguoiduatinGetNDT($path_image = 'images/bm11/')
{
	...
	$page		=	$_REQUEST['page'];
	$link_content	=	$_REQUEST['conten_link'];
	$begin		=	$_REQUEST['begin_get_content'].'<br />';
	$end		=	'<br />'.$_REQUEST['end_get_content'];	

	if (!$id_content) {
		echo $begin;
			echo 'ERROR_GET_CONTENT_NGUOI_DUA_TIN|Not get content id';
		echo $end;
		die();
	}		
	require_once(dirname(__FILE__).DS.'..'.DS.'tables'.DS.'vov_article2010_new2.php');
	...
	mosModelNewsNguoiduatinGetNDT($id_content, $link_content, $page, $path_image, $link_image, $SiteID);
